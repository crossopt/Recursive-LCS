...
do
	...
	do
	    ...
	    ts=$(date +%s%N) ; printf $pattern | ./build/dp_test 2> /dev/null | cat; tt=$((($(date +%s%N) - $ts))) ; time_dp=$((time_dp + tt)); # echo dp $tt
	    ts=$(date +%s%N) ; echo $c $i $pattern | ./build/lz_test; tt=$((($(date +%s%N) - $ts))) ; time_lz=$((time_lz + tt)); # echo lz $tt
	    ts=$(date +%s%N) ; printf $pattern | ./run_agrep.sh 2> /dev/null | cat; tt=$((($(date +%s%N) - $ts))) ; time_agrep=$((time_agrep + tt)); # echo agrep $tt
	done
	# / 20 / 1000000
